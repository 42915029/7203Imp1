...
typedef RT_MODEL_helicopter_integral RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * helicopter_integral(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void helicopter_integral_initialize();
   extern void helicopter_integral_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helicopter_integral_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helicopter_integral_DWork.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helicopter_integral -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  helicopter_integral_initialize();
  S = helicopter_integral_M;
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (helicopter_integral_DWork.HILReadEncoderTimebase_Task, (t_clock)
           helicopter_integral_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
