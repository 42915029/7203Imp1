...
typedef RT_MODEL_q_heli3d_skeleton RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_heli3d_skeleton(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void q_heli3d_skeleton_initialize();
   extern void q_heli3d_skeleton_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_heli3d_skeleton_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_heli3d_skeleton_DWork.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_heli3d_skeleton -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  q_heli3d_skeleton_initialize();
  S = q_heli3d_skeleton_M;
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (q_heli3d_skeleton_DWork.HILReadEncoderTimebase_Task, (t_clock)
           q_heli3d_skeleton_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
